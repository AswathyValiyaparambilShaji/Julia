...
OUTDIR = joinpath(base2, "WindInput")
mkpath(OUTDIR)
...
for xn in cfg["xn_start"]:cfg["xn_end"]
    for yn in cfg["yn_start"]:cfg["yn_end"]
        ...
        # --- Read Wind Stress ---
        taux = Float64.(open(joinpath(base, "oceTAUX", "oceTAUX_v2_$suffix.bin"), "r") do io
            nbytes = nx * ny * nt * sizeof(Float32)
            reshape(reinterpret(Float32, read(io, nbytes)), nx, ny, nt)
        end)


        tauy = Float64.(open(joinpath(base, "oceTAUX", "oceTAUX_v2_$suffix.bin"), "r") do io
            nbytes = nx * ny * nt * sizeof(Float32)
            reshape(reinterpret(Float32, read(io, nbytes)), nx, ny, nt)
        end)
        ...
        # --- Save ---
        wpi_file = joinpath(OUTDIR, "wpi_nt_$suffix.bin")
