...
ax_rough_bar = Axis(fig_bars[1, 2],
    xlabel="Roughness Range",
    ylabel="Area-weighted Residual (W/m²)",
    title="Residual vs Roughness",
    xticklabelrotation=π/4)


# Create x-axis labels for roughness bins
roughness_labels = [@sprintf("%.2e to\n%.2e", roughness_bin_edges[k], roughness_bin_edges[k+1])
                    for k in 1:n_rough_bins]
...
labels_rough = roughness_labels[valid_rough]
