...
uflux_avg_out = fill(NaN, n_points)   # depth-averaged, time-averaged  [W/m^2]
vflux_avg_out = fill(NaN, n_points)
uflux_int_out = fill(NaN, n_points)   # depth-integrated, time-averaged [kW/m]
vflux_int_out = fill(NaN, n_points)
...
@time Threads.@threads for ti in 1:length(tile_keys)
    ...
    for p in point_ids
        ...
        uflux_t = vec(sum(uflux_modes, dims=2))   # summed over 5 modes
        vflux_t = vec(sum(vflux_modes, dims=2))


        uflux_avg_out[p] = mean(uflux_t)                 # depth-avg, time-avg  [W/m^2]
        vflux_avg_out[p] = mean(vflux_t)
        uflux_int_out[p] = mean(uflux_t) * H / 1000       # depth-integrated      [kW/m]
        vflux_int_out[p] = mean(vflux_t) * H / 1000


        println("  Point $p (mooring $(mooring_ids[p])): H=$(round(H,digits=1)) m | " *
                "uflux_avg=$(uflux_avg_out[p]) W/m^2 | uflux_int=$(uflux_int_out[p]) kW/m")
    end
end
...
ufa_v = defVar(ds, "uflux_depth_avg", Float64, ("station",)); ufa_v[:] = uflux_avg_out
ufa_v.attrib["long_name"] = "Depth-averaged, time-averaged 5-mode u-flux"
ufa_v.attrib["units"] = "W/m^2"


vfa_v = defVar(ds, "vflux_depth_avg", Float64, ("station",)); vfa_v[:] = vflux_avg_out
vfa_v.attrib["long_name"] = "Depth-averaged, time-averaged 5-mode v-flux"
vfa_v.attrib["units"] = "W/m^2"


ufi_v = defVar(ds, "uflux_depth_int", Float64, ("station",)); ufi_v[:] = uflux_int_out
ufi_v.attrib["long_name"] = "Depth-integrated, time-averaged 5-mode u-flux (for comparison to IWAP kW/m)"
ufi_v.attrib["units"] = "kW/m"


vfi_v = defVar(ds, "vflux_depth_int", Float64, ("station",)); vfi_v[:] = vflux_int_out
vfi_v.attrib["long_name"] = "Depth-integrated, time-averaged 5-mode v-flux (for comparison to IWAP kW/m)"
