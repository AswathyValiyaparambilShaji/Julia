...
# Negative-only residual for plotting (positive values set to NaN)
Residual_neg = copy(Residual)
Residual_neg[Residual_neg .>= 0] .= NaN
...
hm3 = heatmap!(ax3, lon, lat, WPI_plot;
   interpolate = false,
   colorrange = crange,
   colormap = cmap)

# Row 1, Column 4: Dissipation (negative values only)
ax4 = Axis(fig[1, 4],
   title = "(d) <D> (negative only)",
   xlabel = "",
   xticklabelsvisible = false,
   ylabel = "",
   yticklabelsvisible = false
)
hm4 = heatmap!(ax4, lon, lat, Residual_neg;
   interpolate = false,
...
# Save figure
FIGDIR = cfg["fig_base"]
save(joinpath(FIGDIR, "EnergyBudget_wkly_nt_pos_V1.png"), fig)
