...
Threads.@threads for xn in cfg["xn_start"]:cfg["xn_end"]
    for yn in cfg["yn_start"]:cfg["yn_end"]
        ...
        open(joinpath(base2, "BP", "bp_nt_$suffix.bin"), "w") do io
            write(io, Float32.(dropdims(mean(bp[:, :, t_safe_start:t_safe_end], dims=3), dims=3)))
        end


        BP_3day = zeros(Float32, nx, ny, length(safe_chunks))
        ...
        open(joinpath(base2, "BP_wkly2", "bp_wkly_nt_$suffix.bin"), "w") do io
